        ...
        if h &- len == 1 {
          ...
          for s in 0 ..< 1 << len {
            ...
            for i in 0 ..< p {
              ...
              let a1na3imag: ULL = 1 &* ULL(mint(ull: a1 &+ mod2 &- a3).val) &* ULL(imag.val)
              let na2: ULL = mod2 &- a2
              a[i &+ offset] = mint(ull: a0 &+ a2 &+ a1 &+ a3)
              a[i &+ offset &+ 1 &* p] = mint(ull: a0 &+ a2 &+ (2 * mod2 &- (a1 &+ a3)))
          ...
          for s in 0 ..< 1 << (len &- 2) {
            ...
            for i in 0 ..< p {
              ...
              let a2na3iimag: ULL =
                1
                &* ULL(
                  mint(ull: (umod &+ a2 &- a3) &* ULL(iimag.val))
                    .val)
              a[i &+ offset] = mint(ull: a0 &+ a1 &+ a2 &+ a3)
              a[i &+ offset &+ 1 &* p] =
                mint(
                  ull: (a0 &+ (umod &- a1) &+ a2na3iimag) &* ULL(irot.val))
              a[i &+ offset &+ 2 &* p] =
                mint(
                  ull: (a0 &+ a1 &+ (umod &- a2) &+ (umod &- a3))
                    &* ULL(irot2.val))
              a[i &+ offset &+ 3 &* p] =
                mint(
                  ull: (a0 &+ (umod &- a1) &+ (umod &- a2na3iimag))
                    &* ULL(irot3.val))
            }
            if s &+ 1 != 1 << (len &- 2) {
...
public func convolution_ll(
  ...
  let MOD3: ULL = ULL(mod3.umod)
  let M2M3: ULL = MOD2 &* MOD3
  let M1M3: ULL = MOD1 &* MOD3
  let M1M2: ULL = MOD1 &* MOD2
  let M1M2M3: ULL = MOD1 &* MOD2 &* MOD3

  let i1: ULL = ULL(_Internal.inv_gcd(LL(MOD2) * LL(MOD3), LL(MOD1)).second)
  let i2: ULL = ULL(_Internal.inv_gcd(LL(MOD1) * LL(MOD3), LL(MOD2)).second)
  let i3: ULL = ULL(_Internal.inv_gcd(LL(MOD1) * LL(MOD2), LL(MOD3)).second)
