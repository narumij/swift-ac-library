        ...
        if len == 1 {
          ...
          for s in 0..<(1 << (len - 2)) {
            ...
            for i in 0..<p {
              ...
              let a2na3iimag: ULL =
                1 * ULL(mint(unsigned: (umod &+ a2 &- a3) &* ULL(iimag.val), umod: umod).val)
              a[i + offset] = mint(unsigned: a0 &+ a1 &+ a2 &+ a3, umod: umod)
              a[i + offset + 1 * p] =
                mint(unsigned: (a0 &+ (umod &- a1) &+ a2na3iimag) &* ULL(irot.val), umod: umod)
              a[i + offset + 2 * p] =
                mint(
                  unsigned: (a0 &+ a1 &+ (umod &- a2) &+ (umod &- a3)) &* ULL(irot2.val), umod: umod
                )
              a[i + offset + 3 * p] =
                mint(
                  unsigned: (a0 &+ (umod &- a1) &+ (umod &- a2na3iimag)) &* ULL(irot3.val),
                  umod: umod)
            }
            if s + 1 != 1 << (len - 2) {
    ...
    B: Collection, B.Element == static_modint<mod>
  {
    ...
    if n < m {
      for j in 0..<m {
        for i in 0..<n {
          ans[i + j] +=
            a[a.index(a.startIndex, offsetBy: i)] * b[b.index(b.startIndex, offsetBy: j)]
        }
      }
    } else {
      for i in 0..<n {
