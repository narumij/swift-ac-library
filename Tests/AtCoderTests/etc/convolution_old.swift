    ...
    while len < h {
      if h - len == 1 {
        ...
        for s in 0..<(1 << len) {
          ...
          if s + 1 != (1 << len) { rot *= info.rate2[_Internal.countr_zero(~CUnsignedInt(s))] }
        }
        len += 1
      } else {
        // 4-base
        ...
        for s in 0..<(1 << len) {
          ...
          if s + 1 != (1 << len) { rot *= info.rate3[_Internal.countr_zero(~(CUnsignedInt(s)))] }
        }
        len += 2
      }
    }
      ...
      if len == 1 {
        ...
        for s in 0..<(1 << (len - 1)) {
          ...
          for i in 0..<p {
            ...
            a[i + offset] = l + r
            a[i + offset + p] = mint(
              (ULL(mint.mod) + ULL(l.val) - ULL(r.val)) * ULL(irot.val))
          }
          if s + 1 != (1 << (len - 1)) {
        ...
        for s in 0..<(1 << (len - 2)) {
          ...
          for i in 0..<p {
            ...
            let a2na3iimag: ULL =
              1 * ULL(mint((ULL(mint.mod) + a2 - a3) * ULL(iimag.val)).val)
            a[i + offset] = mint(a0 + a1 + a2 + a3)
            a[i + offset + 1 * p] =
              mint((a0 + (ULL(mint.mod) - a1) + a2na3iimag) * ULL(irot.val))
            a[i + offset + 2 * p] =
              mint((a0 + a1 + (ULL(mint.mod) - a2) + (ULL(mint.mod) - a3)) * ULL(irot2.val))
            a[i + offset + 3 * p] =
              mint((a0 + (ULL(mint.mod) - a1) + (ULL(mint.mod) - a2na3iimag)) * ULL(irot3.val))
          }
          if s + 1 != (1 << (len - 2)) {
