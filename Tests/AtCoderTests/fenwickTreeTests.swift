...
final class fenwickTreeTests: XCTestCase {
  ...
  func testOverFlowULL() throws {
    ...
    for i in 0 ..<= 10 {
      for j in 0 ..<= 10 {
        ...
        if i <= j {
          XCTAssertEqual(
            ((j - i) % 2) != 0 ? ull(bitPattern: 1 << 63) + sum : sum, fw.sum(i, j)
          )
        }
      }
    }
  ...
  func testHandleUnsigned() throws {
    XCTAssertEqual(4_294_967_295, CInt(-1).unsigned)
    XCTAssertEqual(18_446_744_073_709_551_615, CLongLong(-1).unsigned)
    XCTAssertEqual(18_446_744_073_709_551_615, Int(-1).unsigned)
  }
}
